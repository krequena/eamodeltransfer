...
public class TransferProcess {
	...
	private static final String[] tableList = { "t_attributeconstraints", "t_attributetag", "t_connectorconstraint",
			"t_connectortag", "t_document", "t_files", "t_html", "t_image", "t_paletteitem", "t_phase",
			"t_operationtag", "t_rtf", "t_rules", "t_script", "t_secgrouppermission", "t_seclocks", "t_secusergroup",
			"t_secuserpermission", "t_snapshot", "t_taggedvalue", "t_tasks", "t_template", "t_testplans",
			"t_umlpattern", "t_version", "t_xref", "t_xrefsystem", "t_xrefuser", "usys_system", "t_operationparams",
			"t_operationposts", "t_operationpres", "t_issues", "t_attribute", "t_connector", "t_diagramlinks",
			"t_diagramobjects", "t_method", "t_diagram", "t_objectconstraint", "t_objecteffort", "t_objectfiles",
			"t_objectmetrics", "t_objectproblems", "t_objectproperties", "t_objectrequires", "t_objectresource",
			"t_objectrisks", "t_objectscenarios", "t_objecttests", "t_objecttrx", "t_operation", "t_palette",
			"t_secuser", "t_secpermission", "t_secgroup", "t_object", "t_glossary", "t_implement", "t_diagramtypes",
			"t_genopt", "t_ecf", "t_efforttypes", "t_connectortypes", "t_constants", "t_constrainttypes", "t_datatypes",
			"t_authors", "t_cardinality", "t_category", "t_clients", "t_complexitytypes", "t_metrictypes", "t_lists",
			"t_mainttypes", "t_objecttypes", "t_ocf", "t_rtfreport", "t_package", "t_problemtypes", "t_projectroles",
			"t_propertytypes", "t_requiretypes", "t_resources", "t_risktypes", "t_scenariotypes", "t_secpolicies",
			"t_tcf", "t_statustypes", "t_stereotypes", "t_testtypes", "t_trxtypes", "t_testclass", "t_roleconstraint",
			"usystables" };

	private static final String SETTING_ON = "ON";
	private static final String SETTING_OFF = "OFF";
	private static final String SETTING_ENABLE = "ENABLE";
	private static final String SETTING_DISABLE = "DISABLE";

	private static final Logger LOG = LoggerFactory.getLogger(TransferProcess.class);

	/**
	 * Performs the database transfer from the source to the destination
	 * connections.
	 *
	 ...
	 * @param batchInsert      Performs the insertions in batch mode, which enhances
	 *                         the overall performance of the operation.
	 * @throws EAModelTransferException Generic handled exception.
	 */
	public static void modelTransfer(final Connection sourceConnection, final Connection targetConnection,
			final boolean batchInsert) throws EAModelTransferException {
		...
		try {
			...
			for (final String table : tableList) {
				...
				final PreparedStatement insertStatement = prepareInsertStatement(targetConnection, table, metadata);
					...
					for (int column = 1; column <= metadata.getColumnCount(); column++) {
						if (metadata.getColumnType(column) == Types.CHAR
							...
							insertStatement.setDouble(column, resultSet.getDouble(column));
								...
								|| metadata.getColumnType(column) == Types.BINARY) {
							...
							if (resultSet.getBlob(column) == null || resultSet.getBlob(column).length() == 0) {
								...
								insertStatement.setBinaryStream(column, new ByteArrayInputStream(resultSet
										.getBlob(column).getBytes(1L, (int) resultSet.getBlob(column).length())));
							}
						} else {
	...
	/**
	 * Creates a insertion {@link java.sql.PreparedStatement} for a connection given
	 * a specific table.
	 *
	...
	private static PreparedStatement prepareInsertStatement(final Connection connection, final String table,
		...
		// Create the insertion SQL depending on the database. H2 (used in tests)
		// doesn't need the column names nor '[]'.
		...
		final PreparedStatement insertStatement = connection.prepareStatement(insertString);
	 ...
	 * @param value                 If true, enables the setting. Otherwise,
	 *                              disables it.
	 */
	private static void changeIdentityInserts(final Connection destinationConnection, final String tableName,
			final boolean value) {
		try {
			final Statement identityStmt = destinationConnection.createStatement();
			identityStmt.execute((value ? SETTING_DISABLE : SETTING_ENABLE) + " AUTOINCREMENT ON " + tableName);
